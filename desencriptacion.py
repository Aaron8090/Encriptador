...
tgt = 'nopqrstuvwxyzabcdefghijklm'

def rot_13(message):
    rotmapper = message.translate(message.maketrans(orig, tgt))
    print(rotmapper)
    separaMensaje = list(rotmapper)
    ...
    matrizInversa = [[-3/11, 1/11, 7/11], [6/11, -2/11, -3/11], [2/11, 3/11, -1/11]]
    ...
    for zz in range(0, 3):
        for xx in range(0, 3):
            aux = (matrizInversa[0][xx] * numEncriptar[zz]) + (matrizInversa[1][xx] * numEncriptar[zz + 1]) + (matrizInversa[2][xx] * numEncriptar[zz + 2])
    zz+=2
    print("\nEl mensaje desencriptado es: " + mensaje)

def desrot_13(message):
    desrotmapper = message.translate(message.maketrans(tgt, orig))

        
mensaje = input("Ingrese el mensaje: ")
rot_13(mensaje.lower())
desrot_13(mensaje)
